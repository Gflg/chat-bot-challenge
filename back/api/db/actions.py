...
def find_chat_message_by_index_order(db: Session, chat_id: int, index_order: int) -> ChatMessage:
    '''Return a chat message matching the given index order.'''
...
def create_new_chat_message(db: Session, chat_id, chat_message_data: CreateChatMessageInput) -> ChatMessage:
    '''Create a new chat message on database.'''
...
def update_existing_chat_message(
    ...
    '''Update given chat message with new text.'''
...
def update_next_chat_messages(db: Session, chat_id: int, index_order: int):
    '''Update next chat messages based on the index order deleted.'''
